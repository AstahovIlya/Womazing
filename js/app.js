(() => {
    ...
    function initSliders() {
        if (document.querySelector(".body-home__slider")) new core(".body-home__slider", {
    ...
    da.init();
    let script_link = window.location.pathname;
    let menu = document.querySelectorAll(".menu__link");
    menu.forEach((item => {
        if (script_link.indexOf(item.getAttribute("data-menu-link")) !== -1) item.classList.add("_active-menu");
    }));
    let buttonsFilter = document.querySelectorAll(".filter__button");
    for (let button of buttonsFilter) button.addEventListener("click", (function() {
        let buttonAction = this;
        let buttonActive = document.querySelector(".filter__button_active");
        buttonAction.classList.add("filter__button_active");
        if (buttonAction !== buttonActive) buttonActive.classList.remove("filter__button_active");
    }));
    let buttonsPages = document.querySelectorAll(".pages__link");
    for (let button of buttonsPages) button.addEventListener("click", (function(event) {
        event.preventDefault();
        let buttonAction = this;
        let buttonActive = document.querySelector(".pages__link_active");
        buttonAction.classList.add("pages__link_active");
        if (buttonAction !== buttonActive) buttonActive.classList.remove("pages__link_active");
    }));
    window["FLS"] = true;
    isWebp();
    menuInit();
